...
public class OpinionService implements IOpinionService {
    ...
    private VoteEntity getExistingVoteOrNewInstance(UUID userId, UUID productId) {
        Optional<VoteEntity> optionalVote = this.voteRepository.findByUserIdAndProductId(userId, productId);
    ...
    public VotesDTO getProductVotesCount(UUID productId) {
        Integer likes = this.voteRepository.countVoteEntitiesByProductIdEqualsAndLikedEquals(productId, true);
        Integer dislikes = this.voteRepository.countVoteEntitiesByProductIdEqualsAndLikedEquals(productId, false);
    ...
    public void createReview(UUID userId, ReviewDTO reviewDTO) {
        ...
        boolean alreadyExists = reviewRepository.existsByUserIdEqualsAndProductIdEquals(userId, reviewDTO.productId());
        if (alreadyExists) {
            throw new ResourceAlreadyExistsException(
                "The actual user already has a review for the product with the Id: ".concat(reviewDTO.productId().toString())
            );
        }
    ...
    private ReviewEntity getReviewEntity(UUID userId, UUID productId) {
        Optional<ReviewEntity> optionalReviewEntity = this.reviewRepository.findFirstByUserIdEqualsAndProductIdEquals(
                userId,
                productId
        );
